...
public class DownloadService extends Service {
    ...
    public static void startService(Context context, Intent intent) {
        ...
        if (Build.VERSION.SDK_INT < 26) { // || (powerManager != null && powerManager.isIgnoringBatteryOptimizations(intent.getPackage()))) {
            ...
            context.startService(
                    intent); //startForegroundService is not allowed to be called from background from android 12 so we go back to the old way, need to evaluate if this is a problem
        }
    }
    ...
    public synchronized void restore(List<MusicDirectory.Entry> songs, List<MusicDirectory.Entry> toDelete, int currentPlayingIndex,
    ...
    public synchronized DownloadFile forSong(MusicDirectory.Entry song) {
        ...
        for (DownloadFile downloadFile : downloadList) {
            if (downloadFile.getSong().equals(song)) {
                if (((downloadFile.isDownloading() && !downloadFile.isDownloadCancelled() && downloadFile.getPartialFile().exists())
    ...
    public synchronized void checkDownloads() {
        ...
        // Find a suitable target for download.
                ...
                (!downloadList.isEmpty() && remoteState == LOCAL) || !backgroundDownloadList.isEmpty())) {
            currentDownloading = null;
            int n = size();
            ...
            if ((preloaded + 1 == n || preloaded >= Util.getPreloadCount(this) || downloadList.isEmpty() || remoteState != LOCAL)
